...
Example: 'Who is Albert Einstein?' → input: 'Albert Einstein'."""
    ...
    def process_document(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Create vector store - FIX: Use from_texts to avoid callable issue
            try:
                texts = [doc.page_content for doc in chunks]
                metadatas = [doc.metadata for doc in chunks]
                self.vectorstore = FAISS.from_texts(texts, self.embeddings, metadatas=metadatas)
            except Exception as emb_error:
                logger.error(f"Embeddings error with from_texts: {emb_error}")
                # Fallback: recreate embeddings instance
                from langchain.embeddings import OpenAIEmbeddings
